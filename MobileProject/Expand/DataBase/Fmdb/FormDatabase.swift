...
class FormDatabase: NSObject {
    ...
    init(path:String , debugger:Bool ) {
        ...
        databasequeqe?.inDatabase({[weak self] (db) in
            self?.setupFMDBMigrationManager(fmdb: db!)
        })
    ...
    //MARK: --FMDBMigrationManager管理数据库FMDB
    /// FMDBMigrationManager管理数据库FMDB
    func setupFMDBMigrationManager(fmdb:FMDatabase) {
        printLog(message: "fmdb-----:\(fmdb)")
        ...
        //创建HomeInfoObj表 版本从0开始
        let homeInfoObj_expectedPath = Bundle.main.path(forResource: "20170425100911098_CreateHomeInfoObjV1.0", ofType: "sql")
        var homeInfoObj_sql: String?
        do {
            try homeInfoObj_sql = String(contentsOfFile: homeInfoObj_expectedPath!, encoding: String.Encoding.utf8)
            
        } catch let error {
            printLog(message: "homeInfoObj_expectedPath_contentsOfFile:\(error)")
        }
        let migration_1 = Migration(name: "新增HomeInfoObjV1.0表", andVersion: 0, andExecuteUpdate: [homeInfoObj_sql!])
        manager?.addMigration(migration_1)
        
        //HomeInfoObj表 新增locationName字段，版本从1开始
//        let migration_2 = Migration(name: "HomeInfoObj表 新增locationName字段，版本从1开始", andVersion: 1, andExecuteUpdate: ["alter table `HomeInfoObj` add `locationName` text"])
//        manager?.addMigration(migration_2)
        
        //创建FlexValuesObj表 版本从0开始
        let flexValuesObj_expectedPath = Bundle.main.path(forResource: "20170427173115452_CreateFlexValuesObjV1.0", ofType: "sql")
        var flexValuesObj_sql: String?
        do {
            try flexValuesObj_sql = String(contentsOfFile: flexValuesObj_expectedPath!, encoding: String.Encoding.utf8)
            
        } catch let error {
            printLog(message: "flexValuesObj_expectedPath_contentsOfFile:\(error)")
        }
        let migration_3 = Migration(name: "新增FlexValuesObjV1.0表", andVersion: 0, andExecuteUpdate: [flexValuesObj_sql!])
        ...
        if !(manager?.hasMigrationsTable)! {
            do {
                ...
                printLog(message: "createMigrationsTable_error:\(error)")
            }
        }
        do {
            try manager?.migrateDatabase(toVersion: UInt64(UINT16_MAX), progress: nil)
            printLog(message: "migrateDatabase:\(resultState)")
        }catch let error {
...
//            try migration_2?.migrateDatabase(fmdb)
        }catch let error {
            printLog(message: "migrateDatabase_error:\(error)")
        }
    ...
    func query(myclass:AnyClass,columns:Array<String>?,strWhere:String?,strOrderby:String?) ->Array<Any>? {
        ...
        while set.next() {
            ...
            for (key,value) in result! {
                ...
                //String 类型 复制 nil为闪退
//                printLog(message: "key:\(key)------\nvalue:\(value)------\nprovalue:\(String(describing: propertys))")
                if !(value as AnyObject).isEqual(NSNull.init()) {
                    obj.setValue(value, forKey: key as! String)
                    
                }else{
//                    let provalue =  propertys?[key as! String] as! String
                     if let provalue =  propertys?[key as! String] {
                        if (provalue as AnyObject).contains("NSString") {
                            obj.setValue("", forKey: key as! String)
    ...
    func save(entity:AnyObject) -> Bool {
        ...
        if result && tableInfo?._primaryFieldName != nil {
            ...
            printLog(message: "lastId:\(lastId)")
            printLog(message: "tableInfo?._primaryFieldName:\(tableInfo?._primaryFieldName)")
